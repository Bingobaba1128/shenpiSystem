  ...
  <el-card>
    ...
    <el-row :gutter="10" style="padding:0 16px; margin-left:0;margin-right:0;padding-bottom:0" class="specialSelect">
          ...
          <el-row class="special">
            ...
            <el-col :span="6">
              <el-form-item label="出库单号 :">
                <el-input v-model.trim="queryInfo.chuKuNo" placeholder="" clearable maxlength="100" @keyup.enter.native="searchDataM" />
              </el-form-item>
            </el-col>
      ...
      <el-col :lg="3" class="rightPart">
        <el-row style="padding-bottom: 16px" class="function">
          <!-- <el-button type="primary" style="margin-left:16px" @click="addNewRecord">新增入库</el-button> -->

          <el-button class="indexButton" icon="el-icon-search" style="margin-left:0" @click="searchDataM">查询</el-button>
          <el-button class="orangeBtnClass" style="margin-left:16px;float:right" icon="el-icon-refresh" @click="refreshSearch">重置</el-button>

        </el-row>
        <el-row class="function">
          <el-button class="orangeBtnClass" style="float:right;margin-left:104px;" @click="deleteItemList">批量删除</el-button>
    ...
    <el-row style="padding:0 20px;display: flex; flex-direction:column;height:700px">
      ...
      >
        ...
        <el-table-column label="发货日期" prop="signDate" width="150" show-overflow-tooltip />
        <el-table-column label="发货申请单号" prop="applyNo" width="150" show-overflow-tooltip />
        <el-table-column label="出库单号" prop="chuKuNo" width="150" show-overflow-tooltip />
        <el-table-column label="出库日期" prop="chuKuDate" width="150" show-overflow-tooltip />
        ...
        <el-table-column label="总数量" prop="totalQuantity" width="120" show-overflow-tooltip />
        <el-table-column label="录入人" prop="userName" width="120" show-overflow-tooltip />
        <el-table-column label="录入时间" prop="recordDate" width="120" show-overflow-tooltip />
        <el-table-column label="审批状态" prop="approveState" width="120" show-overflow-tooltip />
        <el-table-column label="出库状态" prop="state" width="120" show-overflow-tooltip />

        <el-table-column label="操作" fixed="right" width="300">
          <template slot-scope="scope">
            <el-button type="text" :disabled="scope.row.chuKuNo === null" @click="showDataM(scope.row)">查看</el-button>
            <el-button v-if="scope.row.chuKuNo === null" :disabled="scope.row.approveState !== '待提交' && scope.row.approveState !== '取消' && scope.row.approveState !== '驳回' " type="text" @click="addDataM(scope.row)">出库</el-button>
            <el-button v-if="scope.row.chuKuNo !== null" :disabled="scope.row.approveState !== '待提交' && scope.row.approveState !== '取消' && scope.row.approveState !== '驳回' " type="text" @click="editDataM(scope.row)">出库</el-button>
            ...
            <el-button type="text" @click="downLoadM(scope.row)">下载</el-button>
            <el-button type="text" :style="{color:(scope.row.chuKuNo === null || (scope.row.approveState !== '待提交' && scope.row.approveState !== '取消' && scope.row.approveState !== '驳回')? none:'#FAAB15')}" :disabled="scope.row.chuKuNo === null || (scope.row.approveState !== '待提交' && scope.row.approveState !== '取消' && scope.row.approveState !== '驳回')" @click="deleteData(scope.row.chuKuNo)">删除</el-button>
          </template>
        </el-table-column>
...
import { loadData, searchData, deleteRecord, loadYwDept, loadYeWuPerson, uploadData, passData } from '@/api/xiaoshouchuku'
import { combineObject } from '@/utils/combineObject'
import { booleanList1 } from '@/utils/listVar'
...
export default {
  ...
  methods: {
    ...
    // addNewRecord() {
    //   this.$router.push({ path: '/销售出库新增' })
    // },
    addDataM(data) {
      this.$router.push({ path: '/销售出库/销售出库新增', query: { applyNo: data.applyNo, contractNo: data.contractNo }})
    },
    editDataM(data) {
      this.$router.push({ path: '/销售出库/销售出库修改', query: { chuKuNo: data.chuKuNo, contractNo: data.contractNo, applyNo: data.applyNo }})
    },
    uploadApprove(data) {
      var jsonData = [{
        approveStyle: '11',
        ...
        style_1: '1',
        pkNo: data.chuKuNo,
        url: baseUrl + '/#/销售出库/销售出库审批?chuKuNo=' + data.chuKuNo + '&contractNo=' + data.contractNo
    ...
    downLoadM(data) {
      var param = '/api/ChuKu/addRows?chukuNo=' + data.chuKuNo
      loadData(param).then(res => {
        var url = res.data.data
        var fileLink = baseUrl + url
        window.console.log(fileLink)
        window.open(fileLink)
      })
    },
    showDataM(data) {
      this.$router.push({ path: '/销售出库/销售出库详情', query: { chuKuNo: data.chuKuNo, contractNo: data.contractNo }})
    },
    closeDialog() {
    ...
    deleteData(no) {
      ...
      this.deleteItem = []
      this.$set(deleteItemSingle, 'chuKuNo', no)
    ...
    refreshSearch() {
      ...
      this.$set(this.queryInfo, 'chuKuDate', '')
      ...
      this.$set(this.queryInfo, 'chuKuNo', '')
    ...
    deleteItemList() {
      ...
      this.multipleSelection.map(item => {
        ...
        this.$set(deleteItemSingle, 'chuKuNo', item.chuKuNo)
