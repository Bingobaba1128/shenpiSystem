...
<script>
import { postDataConfig, loadData, loadDept, loadPerson } from '@/api/shenPiPeiZhi'
  ...
  data() {
    return {
      queryInfo: {
        ...
        approveStyleName: this.param.approveStyleName,
        employeeName: this.param.employeeName,
        remark: this.param.remark,
        flag: this.param.flag,
        systemId: this.param.systemId,
        classifyId: this.param.classifyId,
        systemName: this.param.systemName,
        id: this.param.id,
        recordMan: '邓科',
        flagName: ''
  ...
  methods: {
    initBaseData() {
      switch(this.queryInfo.flag){
        case '0':
          this.queryInfo.flagName = '审批'
          break
        case '1':
          this.queryInfo.flagName = '撤单'
          break
        case '2':
          this.queryInfo.flagName = '抄送'
          break          
      }
