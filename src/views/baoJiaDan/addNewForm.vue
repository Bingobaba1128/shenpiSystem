          ...
          <el-scrollbar style="height: calc(100vh - 350px);">
            ...
            <div style=" border: 1px dashed #DCE1F0;">
              <el-row style="padding:0 10px;display: flex; flex-direction:column;padding-bottom:20px;padding-top: 11px;">
                ...
                <el-table :cell-style="changeCellStyle" :data="queryInfo.listM" border class="specialIcon" style="margin-top:12px;font-size:14px" :header-cell-style="{background:'#eef1f6',color:'#606266'}" :span-method="objectSpanMethod">
            ...
            <div style=" border: 1px dashed #DCE1F0;">
              <el-row style="padding:0 10px;display: flex; flex-direction:column;padding-bottom:20px;padding-top: 11px;">
                ...
                <el-table :cell-style="changeCellStyle" :data="queryInfo.listF" border style="margin-top:12px;font-size:14px" :header-cell-style="{background:'#eef1f6',color:'#606266'}" :span-method="objectSpanMethod">
...
import { loadYeWuPerson, loadYwDept, addData, loadKeHuList, loadData, deleteRecord } from '@/api/baoJiaDan'
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      var param2 = '/api/MaterialStyle?flag=0'
      loadData(param2).then(res => {
        var initData = res.data.data
        initData.map((item, index) => {
          this.$set(initData[index], 'classifaction', item.classify)
          this.$set(initData[index], 'yuanCaiLiaoXiaoJi', 0.00)
          this.$set(initData[index], 'name', '')
          this.$set(initData[index], 'id', '')
        })
        this.$set(this.queryInfo, 'listM', initData)
        this.rebuildListM()
      })
      var param3 = '/api/FeeStyle?flag=0'
      loadData(param3).then(res => {
        var initData = res.data.data
        initData.map((item, index) => {
          this.$set(initData[index], 'classifaction', item.classify)
          this.$set(initData[index], 'feeHeJi', 0.00)
          this.$set(initData[index], 'name', '')
          this.$set(initData[index], 'id', '')
        })
        this.$set(this.queryInfo, 'listF', initData)
        this.$set(this.queryInfoO, 'listF', initData)
        this.rebuildListF()
      })
    },
    rebuildListM() {
      if (this.queryInfo.listM.length >= 1) {
        ...
        this.$set(this.queryInfoO, 'listM', this.queryInfo.listM)
    ...
    rebuildListF() {
      if (this.queryInfo.listF.length >= 1) {
        ...
        this.$set(this.queryInfoO, 'listF', this.queryInfo.listF)
      ...
      // this.preSave()
        ...
        addData(this.queryInfo).then(res => {
          if (res.data.code !== 1) {
            ...
            var id = res.data.msg
            this.$store.dispatch('toolbar/delView', this.$route)
            this.$router.push({ path: '/报价单/报价单修改', query: { baoJiaNo: id }})
          }
        })
        // }
      }
