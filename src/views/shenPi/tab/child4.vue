  ...
  <div class="child1">
          ...
          <el-row style="padding-bottom:0">
            ...
            <el-col :span="6">
              <el-form-item label="是否已读 :">
                <el-select v-model="queryInfo.stateName1" placeholder="全部" clearable @clear="searchDataM">
                  <el-option
                    v-for="item in readOrNot"
                    :key="item.name"
                    :label="item.name"
                    :value="item.name"
                    @click.native="readableChange(item.id)"
                  />
                </el-select>
              </el-form-item>
            </el-col>
    ...
    <el-row style="padding:0 20px;display: flex; flex-direction:column;height:700px;margin-top:16px">
      ...
      >
        ...
        <el-table-column label="是否已读" prop="readState" show-overflow-tooltip />
...
<script>
import { loadData, searchData, readableData } from '@/api/shenPi'
...
export default {
  ...
  methods: {
    initData() {
      ...
      loadData(urlParam).then(res => {
        ...
        result.map((item, index) => {
          switch (item.state) {
            case '-5':
              this.$set(result[index], 'readState', '未读')
              break
            case '-6':
              this.$set(result[index], 'readState', '已读')
              break
          }
        })
    ...
    searchDataM() {
      ...
      if (this.queryInfo.stateName1 === '') {
        this.$set(this.queryInfo, 'state', '-7')
      }
    ...
    showDataM(data) {
      this.$router.push({ path: '/审批管理/审批详情', query: { employeeId: data.employeeId, queryInfo: this.queryInfo, employeeName: data.employeeName, pkNo: data.pkNo, url: data.url, currentTab: 'child4', approveNo: data.approveNo, flagSpecial: 1 , status: false}})
      var setReadable = {
        id: data.id
      }
      readableData(setReadable).then(res => {})
    },
    bindDanJuId(id) {
    ...
    refreshSearch() {
      ...
      this.$set(this.queryInfo, 'systemId', '')
      this.$set(this.queryInfo, 'classifyId', '')
      this.$set(this.queryInfo, 'stateName1', '')
      this.$set(this.queryInfo, 'state', '-7')
      this.$set(this.queryInfo, 'approveState', '')
      this.$set(this.queryInfo, 'pkNo', '')
