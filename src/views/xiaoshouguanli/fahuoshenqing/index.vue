    ...
    <el-row :gutter="10" style="padding:0 16px; margin-left:0;margin-right:0;padding-bottom:0" class="specialSelect">
      <el-col :lg="21" style="leftPart">
        <el-form :inline="true" :model="formInline" class="demo-form-inline" label-width="100px" @submit.native.prevent>
          <el-row class="special" style="padding-bottom:16px">
            <el-col :span="6">
    ...
    <el-row style="padding:0 20px;display: flex; flex-direction:column;height:700px">
      ...
      >
        <el-table-column
          ...
          fixed="left"
        />
        <el-table-column type="index" fixed="left" label="序号" width="80" />
        <el-table-column label="申请日期" prop="signDate" width="150" show-overflow-tooltip />
        ...
        <!-- <template slot-scope="scope">
            <p v-for="(item) in scope.row.pinName" :key="item" style="margin:0px">
              {{ item }}
            </p>
          </template>
        </el-table-column> -->
        <el-table-column label="总数量" prop="totalApplyQuantity" width="150" show-overflow-tooltip />
        <el-table-column label="收货客户" prop="clientName" width="150" show-overflow-tooltip />
        ...
        <el-table-column label="操作" fixed="right" width="250">
          <template slot-scope="scope">
            ...
            <el-button type="text" :style="{color:((scope.row.approveState !== '待提交' && scope.row.approveState !== '取消' && scope.row.approveState !== '驳回')? none:'#FAAB15')}" :disabled="(scope.row.approveState !== '待提交' && scope.row.approveState !== '取消' && scope.row.approveState !== '驳回')" @click="deleteData(scope.row.applyNo)">删除</el-button>
          </template>
        </el-table-column>
...
import { loadData, searchData, deleteRecord, loadYwDept, loadYeWuPerson, uploadData, passData } from '@/api/fahuoshenqing'
...
export default {
  ...
  methods: {
    ...
    editDataM(data) {
      this.$router.push({ path: '/发货申请/发货申请修改', query: { applyNo: data.applyNo }})
    },
    downLoadM(data) {
      var param = '/api/Apply/addRows?applyNo=' + data.applyNo
    ...
    showDataM(data) {
      this.$router.push({ path: '/发货申请/发货申请详情', query: { applyNo: data.applyNo }})
    },
    uploadApprove(data) {
      var jsonData = [{
        approveStyle: '9',
        ...
        style_1: '1',
        pkNo: data.applyNo,
        url: baseUrl + '/#/发货申请/发货申请审批?applyNo=' + data.applyNo
      }]
    ...
    deleteData(no) {
      ...
      this.$set(deleteItemSingle, 'applyNo', no)
    ...
    deleteItemList() {
      ...
      this.multipleSelection.map(item => {
        ...
        this.$set(deleteItemSingle, 'applyNo', item.applyNo)
