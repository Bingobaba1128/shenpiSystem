    ...
    <el-row :gutter="10" style="padding:0 20px; margin-left:0;margin-right:0">
      <el-form :inline="true" :model="formInline" class="demo-form-inline" style="display:flex" @submit.native.prevent>
        <el-form-item label="款号:">
          <el-input v-model="queryInfo.kuanHao" placeholder="" clearable @keyup.enter.native="searchDataM" />
        </el-form-item>
...
export default {
  ...
  methods: {
    initData(pageSetting) {
      window.console.log(this.multipleSelectionAll, 'selectAll')
      var url = '/api/Contract/getList?contractNo=' + this.param + '&'
    ...
    saveToServe() {
      ...
      window.console.log(this.param1)
      // if(this.param1)
      for (var i = 0; i < this.multipleSelectionAll.length; i++) {
        for (var j = 0; j < this.param1.length; j++) {
          if (this.multipleSelectionAll[i].id === this.param1[j].id) {
            flag = false
          }
        }
      }
      if (!flag) {
        ...
        this.$emit('closeDialog', this.multipleSelectionAll)
      }
      // var flag = true
